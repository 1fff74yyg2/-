...
public class EntityParaceratherium extends EntityJurassiCraftProtective implements IHerbivore {
    public EntityParaceratherium(EntityType<? extends EntityParaceratherium> type, Level world) {
        super(type, world);
    }

    public EntityParaceratherium(Level world) {
        super(world);
    }
    ...
    protected void dropCustomDeathLoot(DamageSource damageSource, int lootingLevel, boolean recentlyHit) {
        ...
        int count = Math.round(1 + (5.5F * developmentFraction) + this.getRandom().nextInt(1 + (int) (4.0F * developmentFraction)) + this.getRandom().nextInt(1 + lootingLevel));
