...
public class CreatureHerd implements Collection<EntityJurassiCraftCreature> {
    ...
    public Vec3 getPosition(EntityJurassiCraftCreature creature, float f) {
        if (f == 1.0F) {
            return new Vec3(creature.getX(), creature.getY(), creature.getZ());
        } else {
            double x = creature.xOld + (creature.getX() - creature.xOld) * (double) f;
            double y = creature.yOld + (creature.getY() - creature.yOld) * (double) f;
            double z = creature.zOld + (creature.getZ() - creature.zOld) * (double) f;
    ...
    public void attack(LivingEntity target) {
        ...
        {
            if (creature instanceof OwnableEntity) {
                OwnableEntity ownable = (OwnableEntity) creature;
