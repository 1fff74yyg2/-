...
public class JurassiCraftAIPlayfulBaby extends Goal {
    ...
    public boolean canUse() {
        if (this.creature.getRandom().nextInt(this.chance) > 0 || this.creature.isCreatureOlderThan(this.maxAge) || this.creature.isPlaying() || this.creature.isSocializing() || this.creature.getAnimationId() != 0 || this.creature.isSitting() || this.creature.isSleeping() || this.creature.isFleeing() || this.creature.isEating() || this.creature.isDrinking() || this.creature.isAttacking() || this.creature.isDefending())
            return false;
        else {
            ...
            if (!closeCowardList.isEmpty()) {
                for (EntityJurassiCraftCoward entity : closeCowardList) {
                    if (entity != null && entity.getClass() == this.creature.getClass() && entity != this.creature) {
                        double distance = this.creature.distanceToSqr(entity);
    ...
    public void tick() {
        if (!this.creature.getNavigation().isDone())
            this.creature.getNavigation().moveTo(this.otherCreature, this.creature.getCreatureSpeed());
    }

    public boolean canContinueToUse() {
        return this.creature.getRandom().nextBoolean() ? true : this.creature.distanceToSqr(this.otherCreature) > this.minimumDistance;
    }
    ...
    public void stop() {
        ...
        this.otherCreature.getNavigation().stop();
