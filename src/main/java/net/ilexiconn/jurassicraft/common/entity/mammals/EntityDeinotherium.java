...
public class EntityDeinotherium extends EntityJurassiCraftProtective implements IHerbivore {
    public EntityDeinotherium(EntityType<? extends EntityDeinotherium> type, Level world) {
        super(type, world);
    }

    public EntityDeinotherium(Level world) {
        super(world);
    }
    ...
    protected void dropCustomDeathLoot(DamageSource damageSource, int lootingLevel, boolean recentlyHit) {
        ...
        int count = Math.round(1 + (5.0F * developmentFraction) + this.getRandom().nextInt(1 + (int) (4.0F * developmentFraction)) + this.getRandom().nextInt(1 + lootingLevel));
