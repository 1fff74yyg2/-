...
public class EntityArsinoitherium extends EntityJurassiCraftCharges implements IHerbivore {
    public EntityArsinoitherium(EntityType<? extends EntityArsinoitherium> type, Level world) {
        super(type, world);
    }

    public EntityArsinoitherium(Level world) {
        super(world);
    }
    ...
    protected void dropCustomDeathLoot(DamageSource damageSource, int lootingLevel, boolean recentlyHit) {
        ...
        int count = Math.round(1 + (4.0F * developmentFraction) + this.getRandom().nextInt(1 + (int) (3.5F * developmentFraction)) + this.getRandom().nextInt(1 + lootingLevel));
